	...
	public function setup_wizard_header() {
		...
		public function envato_setup_introduction() {
			if ( isset( $_REQUEST['export'] ) ) {
				...
				?>
				<h1><?php printf( __( 'Welcome to the setup wizard for %s.', 'templatation' ), wp_get_theme()); ?></h1>
				<p><?php printf( __( 'It looks like you have already run the setup wizard. Below are some options:', 'templatation' ), wp_get_theme()); ?></p>
				<p style="font-weight: bold;"><?php printf( __( 'Note: To avoid future confusions, Its safe to deactivate and delete %s Demo Importer plugin now.', 'templatation' ), wp_get_theme()); ?></p>
				<ul>
					<li>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-next button-large"><?php _e( 'Run Setup Wizard Again', 'templatation' ); ?></a>
					</li>
<!--					<li>
						<form method="post">
							<input type="hidden" name="reset-font-defaults" value="yes">
							<input type="submit" class="button-primary button button-large button-next" value="<?php /*esc_attr_e( 'Reset font style and colors', 'templatation' ); */?>" name="save_step" />
...
-->				</ul>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(),'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php _e( 'Cancel', 'templatation' ); ?></a>
				</p>
				<?php
			} else {
				?>
				<h1><?php printf( __( 'Welcome to the setup wizard for %s.', 'templatation' ), wp_get_theme()); ?></h1>
				<p><?php  printf( __( 'Thank you for choosing the %s theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, and other setup and tell you a little about Help &amp; Support options. <br/>It should only take 5 minutes.', 'templatation' ), wp_get_theme()); ?></p>
				<p><?php _e( 'No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime(Appearance > Setup Wizard ) if you change your mind!', 'templatation' ); ?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
		...
		private function _content_default_get() {
			...
			if ( is_file( __DIR__.'/content/media.xml' ) ) {
				$content['media'] = array(
					'title' => __( 'Media', 'envato_setup' ),
					'description' => __( 'This will create default pages as seen in the demo.', 'envato_setup' ),
					'pending' => __( 'Pending.', 'envato_setup' ),
					'installing' => __( 'Installing Media.', 'envato_setup' ),
					'success' => __( 'Success.', 'envato_setup' ),
					'install_callback' => array( $this,'_content_install_media' ),
				);
			}
			if ( is_file( __DIR__.'/content/all.xml' ) ) {
				$content['pages'] = array(
			...
			if ( is_file( __DIR__.'/content/posts.xml' ) ) {
				$content['posts'] = array(
					'title' => __( 'Posts', 'envato_setup' ),
					'description' => __( 'This will create default Posts as seen in the demo.', 'envato_setup' ),
					'pending' => __( 'Pending.', 'envato_setup' ),
					'installing' => __( 'Installing Default Posts.', 'envato_setup' ),
					'success' => __( 'Success.', 'envato_setup' ),
					'install_callback' => array( $this,'_content_install_posts' ),
				);
			}
			if ( class_exists( 'woocommerce' ) && is_file( __DIR__.'/content/products.xml' ) ) {
...
/*			$content['menu'] = array(
		...
		public function envato_setup_default_content() {
			...
			<form method="post">
					<p><?php printf( __( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. If you see ajax error below, do not worry and contact support. Please note that Media/Pages might take upto 2-4 minutes, rest will be faster. ', 'templatation' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
		...
		private function _content_install_media() {
			return $this->_import_wordpress_xml_file( __DIR__ .'/content/media.xml' );
		}
		private function _content_install_posts() {
			return $this->_import_wordpress_xml_file( __DIR__ .'/content/posts.xml' );
		}
		...
		private function _content_install_settings() {
		    ...
		    if ( !empty( $theme_options ) || is_array( $theme_options ) ) {
	          ...
	          // Hook before import
	          $data = apply_filters( 'tt_theme_import_theme_options', $theme_options );
	          update_option( 'tt_temptt_opt', $theme_options );
		...
		public function envato_setup_tt_theme_setup() {
			...
			<form method="post">
				<p><?php printf( __( 'On this page, we setup the Homepages and menus as well as some other required setups. Please do not skip this step and click Continue button. ' ,'templatation' ), '' ); ?></p>
		...
		public function envato_setup_tt_theme_setup_save() {
			...
			// Set reading options // Done already above.
/*			$homepge = get_page_by_title( 'Home' );
		...
		public function envato_setup_customize() {
			...
			<p>
				Most changes to the website can be made through the Themeoptions from the WordPress dashboard. These include:
			</p>
			<ul>
				<li>Look: Upload a new logo, change styles, colors etc.</li>
				<li>Layout: Enable/Disable/Change Layouts for header/footer etc.</li>
				<li>Components: Control components of headers/footers like copyright info on footer etc.</li>
				<li>Other Misc settings like social icons, email address etc.</li>
			</ul>
			<p>To change the Sidebars go to Appearance > Widgets. Here widgets can be "drag &amp; droped" into sidebars.</p>
			<p>
				<em>Advanced Users: If you are going to make changes to the theme source code please use a <a href="https://codex.wordpress.org/Child_Themes" target="_blank">Child Theme</a> rather than modifying the main theme HTML/CSS/PHP code. This allows the parent theme to receive updates without overwriting your source code changes. <br/> A blank child theme is supplied with main download and can be found in /resource folder of Main themeforest download zip.</em>
			</p>

			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php _e( 'Continue', 'templatation' ); ?></a>
			</p>
		...
		public function envato_setup_help_support() {
			...
			<p>Item Support can be accessed from <a href="http://templatation.com/support/" target="_blank">http://templatation.com/support/</a> and includes:</p>
			<ul>
			...
			<ul>
				<li>Customization services (this is available as <a href="http://templatation.com/support/" target="_blank"> paid gig</a>)</li>
				<li>Installation & Setup services (this is available as <a href="http://templatation.com/support/" target="_blank"> paid gig</a>)</li>
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php _e( 'Agree and Continue', 'templatation' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
			<?php
		}
		...
		public function envato_setup_ready() {
			...
			?>
			<a href="https://twitter.com/share" class="twitter-share-button" data-url="http://themeforest.net/user/templatation/portfolio?ref=templatation" data-text="<?php echo esc_attr( 'I just installed the ' . wp_get_theme() .' #WordPress theme from #ThemeForest' ); ?>" data-via="EnvatoMarket" data-size="large">Tweet</a>
			<script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src="//platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");</script>

			<h1><?php _e( 'Your Website is Ready!', 'templatation' ); ?></h1>

			<p>Congratulations! The theme has been activated and your website is ready. Login to your WordPress dashboard and go to Theme Options to customize website. Feel free to contact support if you have any question.</p>
			<p style="font-weight: bold;"><?php printf( __( 'Note: To avoid future confusions, Its safe to deactivate and delete %s Demo Importer plugin now.', 'templatation' ), wp_get_theme()); ?></p>
			<p>Please come back and <a href="http://themeforest.net/downloads" target="_blank">leave a 5-star rating</a> if you are happy with this theme. <br/>Follow <a  href="https://twitter.com/templatation" target="_blank">@templatation</a> on Twitter to see updates. Thanks! </p>

			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2><?php _e( 'Next Steps', 'templatation' ); ?></h2>
					<ul>
						<li class="setup-product"><a class="button button-primary button-large" href="https://twitter.com/templatation" target="_blank"><?php _e( 'Follow @templatation on Twitter', 'templatation' ); ?></a></li>
						<li class="setup-product"><a class="button button-next button-large" href="<?php echo esc_url( home_url() ); ?>"><?php _e( 'View your new website!', 'templatation' ); ?></a></li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2><?php _e( 'More Resources', 'templatation' ); ?></h2>
					<ul>
						<li class="documentation"><a href="https://www.gitbook.com/@templatation//" target="_blank"><?php _e( 'Read the Theme Documentation', 'templatation' ); ?></a></li>
						<li class="howto"><a href="https://wordpress.org/support/" target="_blank"><?php _e( 'Learn how to use WordPress', 'templatation' ); ?></a></li>
						<li class="rating"><a href="http://themeforest.net/downloads" target="_blank"><?php _e( 'Leave an Item Rating', 'templatation' ); ?></a></li>
						<li class="support"><a href="http://templatation.com/support/" target="_blank"><?php _e( 'Get Help and Support', 'templatation' ); ?></a></li>
					</ul>
				</div>
			</div>
			<?php
